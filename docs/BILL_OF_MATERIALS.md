...
| 1.6 | USB-A → USB-B cable | 1 | 0.5 to 1 m | Pi ↔ Mega | Serial link + Mega power | 3 |
...
| # | Component | Qty | Key spec | Interface / connection | Purpose | Est. € |
|---|-----------|:--:|----------|------------------------|---------|------:|
| 3.1 | DHT11 temp/humidity module | 1 | 0 to 50 °C ±2 °C, 20 to 90 %RH; 3-pin (on-board pull-up) | 1-wire data → Mega **D3**; 5 V | Cage temperature & humidity | 3 |
| 3.2 | MH-Z19C NDIR CO₂ sensor | 1 | 0 to 5000 ppm, UART 9600, 3.3 V logic, 5 V supply | UART, Mega Serial3 **D14/D15** *or* Pi`/dev/serial0` (GPIO14/15) | Metabolic CO₂ (VCO₂ proxy) | 22 |
| 3.3 | JXW-02 O₂ sensor + CH340 USB board | 1 | Electrochemical, 0 to 25 % O₂, UART 9600 | **USB** → Pi (`/dev/ttyUSB*`, CH340 VID`0x1A86`, auto-detected) | Metabolic O₂ (VO₂ proxy); reads ~20.9 % in air | 35 |
...
| 4.4 | YF-S401 flow sensor | 1 | 0.3 to 6 L/min, **7.5 pulses/mL**, Hall-effect | Signal → Mega **D2** (interrupt); 5 V | Measure delivered volume; closed-loop dose feedback | 5 |
