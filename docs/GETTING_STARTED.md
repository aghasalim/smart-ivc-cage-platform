...
2. **How do I install and run it from the submitted source on a fresh PC?**: a step-by-step "Getting Started" reproduction guide.
...
- **Per-camera settings panel (⚙ gear)**: **18+ adjustable functions**:
  ...
  - *Digital zoom:* 1 to 4× zoom with X/Y pan (client-side, since these cameras
...
| **Python** | 3.11 to 3.13 | backend (FastAPI) |
...
The live demo at **example.org is entirely self-hosted on the Raspberry Pi**: there is no Vercel/Railway.`cloudflared` on the Pi opens an outbound
